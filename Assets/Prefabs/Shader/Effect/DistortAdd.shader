...
Shader "Custom/Effects/DistortAdd" {
Properties {
...
{
	//noise effect
	float2 time_xz = float2(fmod(_Time.x, 240)*_HeatTime,fmod(_Time.z, 240)*_HeatTime);
	float2 time_yx = float2(fmod(_Time.y, 240)*_HeatTime,fmod(_Time.x, 240)*_HeatTime);
	fixed4 offsetColor1 = tex2D(_NoiseTex, i.uvmain + time_xz);
    fixed4 offsetColor2 = tex2D(_NoiseTex, i.uvmain + time_yx);
