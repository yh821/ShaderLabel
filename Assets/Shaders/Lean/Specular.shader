...
// 高光反射
Shader "Lean/Specular"
{ 
    Properties{
        _MainTex ("Texture", 2D) = "white" {}
        _Color ("Color", color) = (1,1,1,1)
        _Specular ("Specular", Color) = (1, 1, 1, 1)//高光反射颜色
        _Gloss("Gloss", Range(1,100)) = 10 //高光的参数
    }
    SubShader{
        Pass {
            ...
            fixed4 frag(v2f f) : SV_Target 
            {
                ...
                // 光照方向
                fixed3 lightDir = normalize(UnityWorldSpaceLightDir(f.worldVertex));//_WorldSpaceLightPos0.xyz;//对于每个顶点来说，光的位置就是光的方向，因为光是平行光
                // 漫反射，Diffuse = 直射光颜色 * max(0, cos(光源方向和法线方向夹角)) * 材质自身色彩
                ...
                // 视野方向 = 摄像机的位置 - 当前点的位置
				fixed3 viewDir = normalize(UnityWorldSpaceViewDir(f.worldVertex));//_WorldSpaceCameraPos.xyz - f.worldVertex
                // 高光反射，Specular = 直射光 * pow(max(0, cos(反射光方向和视野方向的夹角)), 高光反射参数)
